...
LEFT join $tbl_employees te on ttec.emp_id=te.id where ttec.status=1 and te.emp_status=1 ORDER BY ttec.id ASC ";
...
if ($result_minfo ->num_rows > 0) {
    ...
    while($row_minfo  = $result_minfo ->fetch_assoc()) {
        ...
        $sub_array[] =  $row_minfo['indefinite'];
        $sub_array[] = $row_minfo['reasons_temporary_closure'];
        ...
        $sub_array[] = $row_minfo['regulation_property_relations'];
        $sub_array[] =$row_minfo['termination_cases'];
        $sub_array[] = $row_minfo['other_condition_wages'];
        $sub_array[] =$row_minfo['workplace_status'];
        $sub_array[] =$row_minfo['working_conditions'];
        $sub_array[] =$row_minfo['job_description'];
        $sub_array[] =$row_minfo['kvota'];
