...
if ($result_minfo ->num_rows > 0) {
    ...
    while($row_minfo  = $result_minfo ->fetch_assoc()) {
        ...
        $sub_array[] = $row_minfo['indefinite'] ? substr($row_minfo['indefinite'],0,30).'...' : '';
        $sub_array[] = $row_minfo['reasons_temporary_closure'] ? substr($row_minfo['reasons_temporary_closure'],0,30).'...' :'';
        ...
        $sub_array[] = $row_minfo['regulation_property_relations'] ? substr($row_minfo['regulation_property_relations'],0,30).'...' :'';
        $sub_array[] = $row_minfo['termination_cases'] ? substr($row_minfo['termination_cases'],0,30).'...' : '';
        $sub_array[] = $row_minfo['other_condition_wages'] ? substr($row_minfo['other_condition_wages'],0,30).'...' :'';
        $sub_array[] = $row_minfo['workplace_status'] ? substr($row_minfo['workplace_status'],0,30).'...' :'';
        $sub_array[] = $row_minfo['working_conditions'] ? substr($row_minfo['working_conditions'],0,30).'...' :'';
        $sub_array[] = $row_minfo['job_description']? substr($row_minfo['job_description'],0,30).'...' :'';
        $sub_array[] =$row_minfo['kvota']? substr($row_minfo['kvota'],0,30).'...':'';
