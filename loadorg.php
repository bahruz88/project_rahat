...
//            $zNodeArray.array_push({"id":userIdArray[j], "pId":parentArray[j],"name":userArray[j],"open":open});
            if(substr($row_users['code'],0,1)!="P"){
                $sub_array   = array();
                $id=$row_users['id'];
                $sub_array['id'] = $row_users['id'];
                $sub_array['pid'] = ($row_users['parent']);
                $sub_array['name'] = ($row_users['category']);
                ...
                if($result_name){
                    if ($result_name->num_rows > 0) {
                        while($row_name = $result_name->fetch_assoc()) {
                            $sub_array['title'] = ($row_name['full_name']);
                        }
                    }else{
                        $sub_array['title'] = '';
                    }
                 }
                $sub_array['code'] = ($row_users['code']);
                $data[]     = $sub_array;
            }
        }
